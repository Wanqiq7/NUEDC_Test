...
NUEDC Ground Station 是面向无人机竞赛任务的 Web 地面站。Vue 3/Quasar 提供唯一操作
界面，FastAPI Gateway 负责状态与机载通信，独立的 C++17 CLI 只负责 H 题规划。
...
| H 题规划器 | `shared/cpp/` | 无状态 C++17 库与 `build/shared/cpp/h_route_planner_cli`，负责案例、禁飞区、路线、坐标转换和降落几何。 |
| Wire 合约 | `shared/proto/messages.proto` | Python Gateway 与机载端共同使用的 Protobuf wire 源。 |
...
sudo apt update
sudo apt install -y \
  build-essential cmake nlohmann-json3-dev libgtest-dev
```
...
构建产物 `build/shared/cpp/h_route_planner_cli` 通过 stdin 接收
`h_planning_request_v1` JSON，并在 stdout 输出单个 JSON 响应。该进程不保存状态，
也不拥有通信、命令处理或 UI 职责。
...
flowchart TB
    subgraph Ground["Web Ground Station"]
        ...
        Planner["C++17 stateless planner CLI"]
    end
...
题目规划算法属于无状态 planner；消息转换、通信和地面端状态属于 Gateway；交互属于前端。
跨端通信必须使用 `shared/proto/messages.proto`，不得把机载任务状态复制成地面站本地权威。
...
|   `-- cpp
|       |-- include
|       |   `-- h_problem_core    # H 题规划、几何和 CLI 接口
|       |-- src                   # 无状态规划库实现
|       |-- tools                 # planner CLI 入口
|       `-- tests                 # GoogleTest 与 golden 校验
...
- planner 保持无状态，不加入通信、持久状态或 UI 依赖。
- 新赛题的消息转换和界面分别在 Web Gateway 与前端面板中扩展。
- 修改 `shared/proto/messages.proto` 时同步验证 Python Gateway、机载端和 wire golden。
