...
它们在多个入口手工读写，且组合约束散落在任务计划应用、命令确认和运行状态更新中。典型 bug 面是多个状态字段无法一致重置；`missionRuntimeInputs()` 仍需把这些标量拼进 `MissionRuntimeInputs`。
...
3. 规划工作流（`enterNoFlySelectionMode` / `generateTaskPlanFromCandidateSelection` / `applyTaskPlan` / 候选禁飞格选择）。
4. 命令 / ack 状态（③ 处理后收敛为 `AirborneSyncState`）。
5. 持久化接线（`competition::storeTaskPlan`、`DetectionRepository`）。
6. 路径解析（④ 处理）。
...
- `h_problem/mission/h_mission_controller.{h,cpp}` —— 工作流 / 状态持有者。含 `PlanningStateMachine`、`HRoutePlanner`、`MissionCommandService`、`AirborneSyncState`、`DetectionRepository`、当前案例/禁飞格等业务字段，以及规划 / 事件解码 / 持久化逻辑。通过回调或轻接口驱动 view，不直接 new widget。
...
public:
    ...
    virtual void showRoute(const QStringList &no_fly_cells, const QStringList &route, ...) = 0;
...
- 新增 `tests/test_h_mission_controller.cpp`：给定 case + 候选禁飞格，controller 产出规范 `competition::TaskPlan` 并调用 sink 的对应方法（用 mock sink 断言）。
