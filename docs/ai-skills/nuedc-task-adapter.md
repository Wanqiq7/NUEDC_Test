...
```bash
ctest --test-dir build --output-on-failure -R "test_main_window|test_mission_load_adapter|test_task_protocol"
```

## 推荐提示词

请为新题目实现一个地面站 Adapter，保持 `MainWindow` 不变，列出新增文件、Adapter 状态流、任务计划转换和测试用例。
