...
`TaskEvent`、`TaskSummary` 和 Qt 并发元数据不属于规划器闭包，必须删除，除非依赖扫描
证明规划器编译时仍直接需要；若出现这种情况，应先解除依赖，不得因此保留 Qt。
...
| `QSet<T>` | `std::set<T>` 或保持现有迭代语义的等价容器 |
...
- 测试启用时使用 `find_package(GTest CONFIG REQUIRED)` 或发行版提供的等价 CMake
  package，并通过 `gtest_discover_tests` 注册。
