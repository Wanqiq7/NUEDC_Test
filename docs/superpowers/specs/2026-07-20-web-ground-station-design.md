...
- 开发模式与比赛模式。
- 可选 PID 调试工作区，消费现有 UDP JSON 诊断流。
...
PID 调试直接复用机载现有 UDP JSON：默认 10 Hz、端口 9870，包含 X/Y/Z 的 target、
measurement、error、P/I/D 和 output。不再使用 PlotJuggler。

能力由环境变量显式启用：

```bash
NUEDC_PID_DEBUG_ENABLED=1
```

启用时，Gateway 监听 UDP、维护最近窗口、经 WebSocket 推送并可写 PID JSONL；前端
显示独立全屏 PID 调试工作区。超过 500 ms 未收到报文时显示 stale。禁用时不创建
UDP socket、不分配环形缓冲、不写 PID 日志，比赛主界面也不显示入口。比赛启动默认
禁用，调试启动显式启用；机载端调试目标设为 `10.42.0.1:9870`。
...
插件。PID 等高频序列放入专用 composable，避免整棵 store 高频重渲染。
...
PID 调试工作区在小屏一次显示一个轴，允许切换 X/Y/Z。更窄屏幕只保证只读诊断，
不作为第一版关键控制目标。
