...
- Adapter 发起的任务下发和生命周期视觉复位必须在完成既有 ACK/同步状态处理后，把完整的
  `CommandSendResult`（包括失败详情）通知命令监视器；不得将结果压缩为布尔值。
- 生命周期视觉复位的通知不得在辅助函数内部抢先发出。任务替换、ACK 清理、运行态停止或汇总完成的
  调用方必须先完成整个同步状态转换，再通知命令监视器，避免窗口以旧任务状态处理新结果。
...
- 心跳开始时必须捕获当前健康结果世代。若一个更晚完成的用户命令或 Adapter 命令已更新该世代，
  则该旧心跳结果不得覆盖新的 ACK 状态，也不得发出过期健康快照。
- 每一个已接受的心跳或外部命令健康更新都必须生成严格递增的快照世代，并随 `healthChanged` 传递。
  `MainWindow` 仅应用比其最后应用世代更新的快照，防止先入 Qt 队列的旧心跳信号在新命令 ACK 后
  到达并覆盖 UI 状态。
...
10. 任务下发或生命周期命令失败时，监视器收到完整失败详情并只增加一次失败计数；Adapter 现有
    任务同步状态保持其原有失败处理，不被直接强制为离线。
11. 替换任务或完成任务时的生命周期视觉复位结果只在新任务/最终状态已写入后到达窗口，不得让
    监视器或控制按钮短暂观察到旧任务 ACK 状态。
12. 在心跳执行期间收到成功用户命令后，随后返回的旧心跳失败不会把 `Online` 降回 `Checking` 或
    `Offline`；下一次新世代心跳才可更新健康状态。
13. 已排队但较旧的 heartbeat `healthChanged` 信号在新的外部命令 ACK 已应用到窗口后送达时，窗口
    必须忽略它，持续显示较新 ACK 对应的状态。
