...
## 项目结构与模块组织
- `proto/`：Protobuf 协议定义；修改 `messages.proto` 后需重新构建生成代码。
- `python/uav_testbed/`：机载模拟端与航线规划核心逻辑；`python/uav_testbed/generated/` 为生成文件，勿手改。
- `python/tests/`：Python `unittest` 用例，文件命名遵循 `test_*.py`。
- `ground_station/src/`：Qt6 地面站源码；`ground_station/tests/`：Qt Test 单元测试。
- `cases/`：固定联调案例；`build/`：本地构建产物，不应作为手工源码目录。
...
- `python3 -m unittest discover -s python/tests -v`：运行全部 Python 单元测试。
- `./build/ground_station/ground_station_app`：启动地面站。
- `PYTHONPATH=python python3 -m uav_testbed.run_simulator --case cases/sample_case.json`：启动模拟端并加载样例案例。
...
- Qt 测试基于 `Qt6::Test`，新增测试文件放入 `ground_station/tests/test_*.cpp`。
...
## 配置与生成文件提示
- 不要直接编辑 `build/generated/proto/` 或 `python/uav_testbed/generated/` 下的生成文件。
